...
# Обновление студента (добавление в группу)
...
# Вставка предметов (оптимизировано: цикл вместо дублирования)
subjects = ['математика', 'физика']
subject_ids = []
for subject in subjects:
    cursor.execute("INSERT INTO subjects (title) VALUES (%s)", (subject,))
    subject_ids.append(cursor.lastrowid)
    print(f"Добавлен предмет '{subject}' с ID: {subject_ids[-1]}")

# Назначение ID предметам для удобства
subject_math_id = subject_ids[0]  # математика
subject_physics_id = subject_ids[1]  # физика

# Вставка уроков (уже в цикле — хорошо!)
lesson_ids = []
...
db.commit()  # Сохранение всех изменений

# SELECT-запросы (без изменений — они корректны)
query = '''
...
query = '''
SELECT * FROM students JOIN books ON students.id = books.taken_by_student_id WHERE  name = 'Nikolay'
...
query = '''
SELECT
students.name, students.second_name, groups.id, groups.title, books.title, lessons.title, subjects.title, marks.value
FROM students
...
WHERE students.id = %s ORDER BY students.id DESC LIMIT 1
'''
